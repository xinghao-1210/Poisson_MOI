...
print(y)
print('Percentage transduction(%%) (%s): %f' %(v_name, y)) #Percentage-transduction （Cell number selection/NO selection）
...
plt.figure()
plt.plot(n,p,'o--')
plt.plot(1,p1,'o',markersize=7,color='r')
plt.plot([1,1],[0,p1],color='r',ls='--',linewidth=2)
plt.plot([1,0],[p1,p1],color='r',ls='--',linewidth=2)
plt.annotate(('(%f,%f)'%(1,p1)),
            xy=(1,p1),xycoords='data',
            xytext=(+10, +30), textcoords='offset points', fontsize=12,
            arrowprops=dict(arrowstyle="->", connectionstyle="arc3,rad=.2"))

plt.title('Poisson: MOI=%s (%s)' %(str(moi_x),v_name))
